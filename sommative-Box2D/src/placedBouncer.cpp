...
placedBouncer::placedBouncer(Game& game_, sf::RenderWindow& window_, sf::Vector2f start_, sf::Vector2f end_) : game(game_), window(window_)
{
    init(start_, end_);
}

void placedBouncer::init(sf::Vector2f start_, sf::Vector2f end_) {
	sf::Vector2f mouseLine(end_.x - start_.x, end_.y - start_.y);
    ...
    float worldAngle = SFML_Utilities::vecAngle(sf::Vector2f(1.0f, 0.0f), mouseLine);
